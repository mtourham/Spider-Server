...
cgi_handler::cgi_handler(current_server server_config, Request request, int port)
    : _server_config(server_config), _request(request),
    _location(_server_config.getServerLocationsObject()[_request.get_start_line().current_location_index])
{
    init_env(port);
}
...
void            cgi_handler::init_env(int port)
{
    ...
    _env.push_back(std::string("SCRIPT_NAME=") + _location.parseCgiPathObject(start_line.full_path));
    
    if (headers.find("Hostname") != headers.end())
        _env.push_back(std::string("SERVER_NAME=") + headers["Hostname"]);
    else
        _env.push_back(std::string("SERVER_NAME=") + std::to_string(_server_config.parseHostObject()));
...
void cgi_handler::exec(response & response)
{
   pid_t       cgi_pid;
    int         fd_in, fd_out;
    char**      env;
    ...
    else if (cgi_pid == 0)
    {
        ...
        std::string cgi_path = _location.parseCgiPathObject(_request.get_start_line().full_path);
        char * const argv[3] = {
            (char *) cgi_path.c_str(),
        ...
        execve(_location.parseCgiPathObject(_request.get_start_line().full_path).c_str(), argv, env);
...
void cgi_handler::generate_response(std::string &cgi_response, response &response) {
    size_t pos = 0;
    size_t i = 0;
    // size_t j;
...
void cgi_handler::set_internal_server_error(response &response) {
    response.setstatusCode("500");
    response.setstatusDescription("Internal Server Error");
    response.setContentType("text/html");
    response.setBody("<h1>Internal Server Error</h1>");
...
void cgi_handler::parse_header(const std::string &header, response &response) {
    size_t j;
    std::string element;
...
void cgi_handler::set_content_length(response &response) {
